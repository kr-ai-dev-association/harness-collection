...
LLM이 생성한 **FastAPI + SQLAlchemy** 코드에서 반복적으로 나오던 결함을, 코드 내보내기 전에 잡는 정적 검사기. 표준 라이브러리 `ast`만 사용하며 의존성·빌드가 없다.

## 사용
```bash
python python-harness/check.py [TARGET_DIR]   # 기본값: 현재 디렉터리
```
