...
`e2e-harness` 래퍼를 호출하면 **첫 실행 때만 자동으로 `npm install` + 빌드**하고, 이후엔 건너뜁니다(소스가 바뀌면 자동 재빌드). 대상 앱 디렉터리에서 바로:

```bash
/path/to/e2e-llm-harness/e2e-harness run --cwd . --workers 1 --out result.md
```
